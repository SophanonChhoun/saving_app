...
class SavingScreen extends StatefulWidget {
  ...
  const SavingScreen({Key key, this.saving}) : super(key: key);
...
class _SavingScreenState extends State<SavingScreen> {
  ...
  SavingRepo _savingRepo = SavingRepo();
  ...
  void initState() {
    ...
    super.initState();
    _amountCtr.text =
        widget.saving.amount != null ? widget.saving.amount.toString() : "0";
    _noteCtrl.text = widget.saving.note;
    _transactionNameCtrl.text = widget.saving.title;
    if (widget.saving.date != null) {
      _dateTime = widget.saving.date;
    } else {
        ...
        onPressed: () async {
          if (_amountCtr.text != '' ||
            ...
            if (widget.saving.id != null) {
              var result = await _savingRepo.updatedSaving(
                  widget.saving.id,
                  _transactionNameCtrl.text,
                  _noteCtrl.text,
                  _amountCtr.text,
              ...
              var result = await _savingRepo.createSavings(
                  _transactionNameCtrl.text,
                  _noteCtrl.text,
                  _amountCtr.text,
                  _dateTime.toString());
